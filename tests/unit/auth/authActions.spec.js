...
const fakeFirebase = {
  ...
  auth: () => ({
    ...
    currentUser: {
      updateProfile: update => Promise.resolve(update),
      updateEmail: update => Promise.resolve(update),
      reload: update => Promise.resolve(update),
      linkWithCredential: update => Promise.resolve(update),
    },
    getRedirectResult: () => Promise.resolve({ uid: 'asdfasdf' }),
    signOut: () =>
      Promise.resolve({}),
    createUserWithEmailAndPassword: email =>
      email.indexOf('error') !== -1 // eslint-disable-line no-nested-ternary
    ...
    signInWithEmailAndPassword: email =>
      email.indexOf('error2') !== -1 // eslint-disable-line no-nested-ternary
        ? Promise.reject(new Error('asdfasdf'))
        : email === 'error3'
          ? Promise.reject(new Error('auth/user-not-found'))
          : Promise.resolve({ uid: '123', email: 'test@test.com', providerData: [{}] }),
    sendPasswordResetEmail: email =>
      email === 'error' // eslint-disable-line no-nested-ternary
    ...
    confirmPasswordReset: (code, password) =>
      password === 'error'
        ? Promise.reject({ code }) // eslint-disable-line prefer-promise-reject-errors
        : Promise.resolve(),
    verifyPasswordResetCode: code => code === 'error' // eslint-disable-line no-nested-ternary
...
describe('Actions: Auth', () => {
  ...
  describe('confirmPasswordReset', () => {
    it('resets password for real user', () => confirmPasswordReset(dispatch, fakeFirebase, 'test', 'test')
        .then((err) => {
  ...
  describe('updateProfile', () => {
    it('resolves for valid code', async () => {
      const updateObj = { displayName: 'test' };
      res = await updateProfile(dispatch, firebase, updateObj);
      expect(res).to.exist;
      expect(res).to.respondTo('val');
      expect(res.val()).to.have.property('displayName', updateObj.displayName);
    });
  });

  describe('updateAuth', () => {
    it('updates Firebase auth', async () => {
      fakeFirebase._.authUid =
      res = await updateAuth(dispatch, fakeFirebase, { displayName: 'test' }, false);
      expect(res).to.equal('success');
    });

    it('updates profile within database if updateInProfile provided', async () => {
      res = await updateAuth(dispatch, fakeFirebase, { displayName: 'test' }, false);
      expect(res).to.equal('success');
    });
  });
  ...
  describe('reloadAuth', () => {
    it('resolves for valid code', async () => {
      res = await reloadAuth(dispatch, fakeFirebase, 'test');
      expect(res).to.equal('success');
    });
  });
  ...
  describe('linkWithCredential', () => {
    it('resolves for valid code', async () => {
      res = await linkWithCredential(dispatch, fakeFirebase, 'test');
      expect(res).to.equal('success');
    });
  });
});
